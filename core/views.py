...
from core.serializer import VulnerabilidadesSerializer
...
from core.models import Scan, IP, Rede, FfufComandos, Diretorios, Porta, CVE_IP, CVE, SistemaOperacional, Sistema_IP, \
    ...
    Vulnerabilidades_Definicoes, Hostname, Hostname_IP, Checklist, Hash_Senha_Cofre, Senha_Criptografada, Arquivo, \
    ChecklistRede, PostagensForum, EvidenciaCheckList, Vulnerabilidades_Tratamento
...
def ataque_rede(request,rede):
    ...
    if rede == "WQFQWFUQWHFQWHFQWHFIWIF":
        ...
        sistemas = []
        class cves_pegou:
            def __init__(self, sistema, cves):
                self.sistema = sistema
                self.cves = cves
        cves_pegou_vetor = []
        for ip in IP.objects.filter(rede=rede.rede):
            print("aqui")
            print(ip)
            try:
                if Sistema_IP.objects.get(ip__ip=ip.ip,ip__rede=rede.rede,posicao=1) in sistemas:
                    pass
                else:

                    print(Sistema_IP.objects.get(ip__ip=ip.ip,ip__rede=rede.rede,posicao=1))
                    sistemas.append(Sistema_IP.objects.get(ip__ip=ip.ip,ip__rede=rede.rede,posicao=1))
                    cves = pegarCves(Sistema_IP.objects.get(ip__ip=ip.ip,ip__rede=rede.rede,posicao=1).sistema,"escalation")
                    cves_pegou_vetor.append(cves_pegou(Sistema_IP.objects.get(ip__ip=ip.ip,ip__rede=rede.rede,posicao=1),cves))
            except:
                pass

        dados = {"Etapas":sistemas,"cves":cves_pegou_vetor}
        return render(request,'pentest_escolha.html',dados)


def pegarCves(sistema,query):
    import requests
    from bs4 import BeautifulSoup

    vgm_url = f'https://cve.mitre.org/cgi-bin/cvekey.cgi?keyword={sistema}+{query}'
    html_text = requests.get(vgm_url).text
    soup = BeautifulSoup(html_text, 'html.parser')

    cves = []
    for link in soup.find_all('a'):
        if "/cgi-bin/cvename.cgi?name=" == link.get('href')[0:len("/cgi-bin/cvename.cgi?name=")]:
            print(link.get('href').replace("/cgi-bin/cvename.cgi?name=", ""))
            cves.append(link.get('href').replace("/cgi-bin/cvename.cgi?name=", ""))
    return cves
...
@login_required(login_url='/login/')
def selecionar_texto(request,id):
    objeto = ChecklistRede.objects.get(id=id)
    checando = objeto.check
...
def ataque_rede(request,rede):
    ...
    if rede == "WQFQWFUQWHFQWHFQWHFIWIF":
        try:
            rede = Pentest_Rede.objects.filter(usuario=usuario)[0].rede.rede
            print(rede)
            return redirect('/ataque/rede/'+rede)
        except:
            return HttpResponse("crie uma rede associada a esse usuário")
    else:
        rede = requests.utils.unquote(rede)
        ...
        contador = 0
        for a in Checklist.objects.all():
            print(a)
            try:
                print("entrei")
                try:
                    ...
                    if a.parent == None:
                        print("entrei3")
                        ChecklistRede.objects.get(rede=rede_objeto,
                                                  name=str(a.name) +" ("+ str(rede_objeto.rede)+")",
                                                  )
                    else:
                        print("entrei4")
                        print(ChecklistRede.objects.get(name = str(a.parent.name)+" ("+ str(rede_objeto.rede)+")",rede=rede_objeto))
                        parent = ChecklistRede.objects.get(name = str(a.parent.name)+" ("+ str(rede_objeto.rede)+")",rede=rede_objeto)
                        try:
                            ChecklistRede.objects.get(rede=rede_objeto,
                                                      name=str(a.name)+" ("+ str(rede_objeto.rede)+")",
                                                      parent=parent
                            ...
                            ChecklistRede.objects.create(rede=rede_objeto,
        ...
        vulnerabilidades = Vulnerabilidades.objects.filter(ip__rede =rede_objeto)

        dados = {'redepentest':rede_pentest, 'hostnames':hostnames,'vulnerabilidades':vulnerabilidades,
                 'queryparameteres':queryparameteres,'genres': ChecklistRede.objects.filter(rede=rede_objeto),
                 'ips': ips_ativos,'ips_desligados':ips_desligados,'diretorios':diretorios,'rede':rede_objeto,'redes':redes,'whatwebTotal':whatwebTotal,'portas':Porta.objects.filter(ativo=1),'rede_vpn':rede_objeto,'portas_quantidades':portas_quantidades}
        return render(request,'ataque_rede.html',dados)
...
def pentestweb():
    try:
        root = Checklist.objects.create(name="Pentest Web")
    except:
        root = Checklist.objects.get(name="Pentest Web")

    try:

        Checklist.objects.create(name='Foreword by Eoin Keary', parent=root)
        Checklist.objects.create(name='Frontispiece', parent=root)
        Checklist.objects.create(name='Introduction', parent=root)
        Checklist.objects.create(name='The OWASP Testing Project', parent=Checklist.objects.get(name='Introduction'))
        Checklist.objects.create(name='Principles of Testing',
                                 parent=Checklist.objects.get(name='The OWASP Testing Project'))
        Checklist.objects.create(name='Testing Techniques Explained',
                                 parent=Checklist.objects.get(name='Principles of Testing'))
        Checklist.objects.create(name='Manual Inspections and Reviews',
                                 parent=Checklist.objects.get(name='Testing Techniques Explained'))
        Checklist.objects.create(name='Threat Modeling',
                                 parent=Checklist.objects.get(name='Manual Inspections and Reviews'))
        Checklist.objects.create(name='Source Code Review', parent=Checklist.objects.get(name='Threat Modeling'))
        Checklist.objects.create(name='Penetration Testing', parent=Checklist.objects.get(name='Source Code Review'))
        Checklist.objects.create(name='The Need for a Balanced Approach',
                                 parent=Checklist.objects.get(name='Penetration Testing'))
        Checklist.objects.create(name='Deriving Security Test Requirements',
                                 parent=Checklist.objects.get(name='The Need for a Balanced Approach'))
        Checklist.objects.create(name='Security Tests Integrated in Development and Testing Workflows',
                                 parent=Checklist.objects.get(name='Deriving Security Test Requirements'))
        Checklist.objects.create(name='Security Test Data Analysis and Reporting', parent=Checklist.objects.get(
            name='Security Tests Integrated in Development and Testing Workflows'))
        Checklist.objects.create(name='The OWASP Testing Framework', parent=root)
        Checklist.objects.create(name='The Web Security Testing Framework',
                                 parent=Checklist.objects.get(name='The OWASP Testing Framework'))
        Checklist.objects.create(name='Phase 1 Before Development Begins',
                                 parent=Checklist.objects.get(name='The Web Security Testing Framework'))
        Checklist.objects.create(name='Phase 2 During Definition and Design',
                                 parent=Checklist.objects.get(name='Phase 1 Before Development Begins'))
        Checklist.objects.create(name='Phase 3 During Development',
                                 parent=Checklist.objects.get(name='Phase 2 During Definition and Design'))
        Checklist.objects.create(name='Phase 4 During Deployment',
                                 parent=Checklist.objects.get(name='Phase 3 During Development'))
        Checklist.objects.create(name='Phase 5 During Maintenance and Operations',
                                 parent=Checklist.objects.get(name='Phase 4 During Deployment'))
        Checklist.objects.create(name='A Typical SDLC Testing Workflow',
                                 parent=Checklist.objects.get(name='Phase 5 During Maintenance and Operations'))
        Checklist.objects.create(name='Penetration Testing Methodologies',
                                 parent=Checklist.objects.get(name='A Typical SDLC Testing Workflow'))
        Checklist.objects.create(name='Web Application Security Testing', parent=root)
        Checklist.objects.create(name='Introduction and Objectives',
                                 parent=Checklist.objects.get(name='Web Application Security Testing'))
        Checklist.objects.create(name='Information Gathering',
                                 parent=Checklist.objects.get(name='Introduction and Objectives'))
        Checklist.objects.create(name='Conduct Search Engine Discovery Reconnaissance for Information Leakage',
                                 parent=Checklist.objects.get(name='Information Gathering'))
        Checklist.objects.create(name='Fingerprint Web Server', parent=Checklist.objects.get(name='Information Gathering'))
        Checklist.objects.create(name='Review Webserver Metafiles for Information Leakage',
                                 parent=Checklist.objects.get(name='Information Gathering'))
        Checklist.objects.create(name='Enumerate Applications on Webserver',
                                 parent=Checklist.objects.get(name='Information Gathering'))
        Checklist.objects.create(name='Review Webpage Content for Information Leakage',
                                 parent=Checklist.objects.get(name='Information Gathering'))
        Checklist.objects.create(name='Identify Application Entry Points',
                                 parent=Checklist.objects.get(name='Information Gathering'))
        Checklist.objects.create(name='Map Execution Paths Through Application',
                                 parent=Checklist.objects.get(name='Information Gathering'))
        Checklist.objects.create(name='Fingerprint Web Application Framework',
                                 parent=Checklist.objects.get(name='Information Gathering'))
        Checklist.objects.create(name='Fingerprint Web Application',
                                 parent=Checklist.objects.get(name='Information Gathering'))
        Checklist.objects.create(name='Map Application Architecture',
                                 parent=Checklist.objects.get(name='Information Gathering'))
        Checklist.objects.create(name='Configuration and Deployment Management Testing',
                                 parent=Checklist.objects.get(name='Map Application Architecture'))
        Checklist.objects.create(name='Test Network Infrastructure Configuration',
                                 parent=Checklist.objects.get(name='Configuration and Deployment Management Testing'))
        Checklist.objects.create(name='Test Application Platform Configuration',
                                 parent=Checklist.objects.get(name='Configuration and Deployment Management Testing'))
        Checklist.objects.create(name='Test File Extensions Handling for Sensitive Information',
                                 parent=Checklist.objects.get(name='Configuration and Deployment Management Testing'))
        Checklist.objects.create(name='Review Old Backup and Unreferenced Files for Sensitive Information',
                                 parent=Checklist.objects.get(name='Configuration and Deployment Management Testing'))
        Checklist.objects.create(name='Enumerate Infrastructure and Application Admin Interfaces',
                                 parent=Checklist.objects.get(name='Configuration and Deployment Management Testing'))
        Checklist.objects.create(name='Test HTTP Methods',
                                 parent=Checklist.objects.get(name='Configuration and Deployment Management Testing'))
        Checklist.objects.create(name='Test HTTP Strict Transport Security',
                                 parent=Checklist.objects.get(name='Configuration and Deployment Management Testing'))
        Checklist.objects.create(name='Test RIA Cross Domain Policy',
                                 parent=Checklist.objects.get(name='Configuration and Deployment Management Testing'))
        Checklist.objects.create(name='Test File Permission',
                                 parent=Checklist.objects.get(name='Configuration and Deployment Management Testing'))
        Checklist.objects.create(name='Test for Subdomain Takeover',
                                 parent=Checklist.objects.get(name='Configuration and Deployment Management Testing'))
        Checklist.objects.create(name='Test Cloud Storage',
                                 parent=Checklist.objects.get(name='Configuration and Deployment Management Testing'))
        Checklist.objects.create(name='Identity Management Testing',
                                 parent=Checklist.objects.get(name='Test Cloud Storage'))
        Checklist.objects.create(name='Test Role Definitions',
                                 parent=Checklist.objects.get(name='Identity Management Testing'))
        Checklist.objects.create(name='Test User Registration Process',
                                 parent=Checklist.objects.get(name='Identity Management Testing'))
        Checklist.objects.create(name='Test Account Provisioning Process',
                                 parent=Checklist.objects.get(name='Identity Management Testing'))
        Checklist.objects.create(name='Testing for Account Enumeration and Guessable User Account',
                                 parent=Checklist.objects.get(name='Identity Management Testing'))
        Checklist.objects.create(name='Testing for Weak or Unenforced Username Policy',
                                 parent=Checklist.objects.get(name='Identity Management Testing'))
        Checklist.objects.create(name='Authentication Testing',
                                 parent=Checklist.objects.get(name='Testing for Weak or Unenforced Username Policy'))
        Checklist.objects.create(name='Testing for Credentials Transported over an Encrypted Channel',
                                 parent=Checklist.objects.get(name='Authentication Testing'))
        Checklist.objects.create(name='Testing for Default Credentials',
                                 parent=Checklist.objects.get(name='Authentication Testing'))
        Checklist.objects.create(name='Testing for Weak Lock Out Mechanism',
                                 parent=Checklist.objects.get(name='Authentication Testing'))
        Checklist.objects.create(name='Testing for Bypassing Authentication Schema',
                                 parent=Checklist.objects.get(name='Authentication Testing'))
        Checklist.objects.create(name='Testing for Vulnerable Remember Password',
                                 parent=Checklist.objects.get(name='Authentication Testing'))
        Checklist.objects.create(name='Testing for Browser Cache Weaknesses',
                                 parent=Checklist.objects.get(name='Authentication Testing'))
        Checklist.objects.create(name='Testing for Weak Password Policy',
                                 parent=Checklist.objects.get(name='Authentication Testing'))
        Checklist.objects.create(name='Testing for Weak Security Question Answer',
                                 parent=Checklist.objects.get(name='Authentication Testing'))
        Checklist.objects.create(name='Testing for Weak Password Change or Reset Functionalities',
                                 parent=Checklist.objects.get(name='Authentication Testing'))
        Checklist.objects.create(name='Testing for Weaker Authentication in Alternative Channel',
                                 parent=Checklist.objects.get(name='Authentication Testing'))
        Checklist.objects.create(name='Authorization Testing', parent=Checklist.objects.get(
            name='Testing for Weaker Authentication in Alternative Channel'))
        Checklist.objects.create(name='Testing Directory Traversal File Include',
                                 parent=Checklist.objects.get(name='Authorization Testing'))
        Checklist.objects.create(name='Testing for Bypassing Authorization Schema',
                                 parent=Checklist.objects.get(name='Authorization Testing'))
        Checklist.objects.create(name='Testing for Privilege Escalation',
                                 parent=Checklist.objects.get(name='Authorization Testing'))
        Checklist.objects.create(name='Testing for Insecure Direct Object References',
                                 parent=Checklist.objects.get(name='Authorization Testing'))
        Checklist.objects.create(name='Session Management Testing',
                                 parent=Checklist.objects.get(name='Testing for Insecure Direct Object References'))
        Checklist.objects.create(name='Testing for Session Management Schema',
                                 parent=Checklist.objects.get(name='Session Management Testing'))
        Checklist.objects.create(name='Testing for Cookies Attributes',
                                 parent=Checklist.objects.get(name='Session Management Testing'))
        Checklist.objects.create(name='Testing for Session Fixation',
                                 parent=Checklist.objects.get(name='Session Management Testing'))
        Checklist.objects.create(name='Testing for Exposed Session Variables',
                                 parent=Checklist.objects.get(name='Session Management Testing'))
        Checklist.objects.create(name='Testing for Cross Site Request Forgery',
                                 parent=Checklist.objects.get(name='Session Management Testing'))
        Checklist.objects.create(name='Testing for Logout Functionality',
                                 parent=Checklist.objects.get(name='Session Management Testing'))
        Checklist.objects.create(name='Testing Session Timeout',
                                 parent=Checklist.objects.get(name='Session Management Testing'))
        Checklist.objects.create(name='Testing for Session Puzzling',
                                 parent=Checklist.objects.get(name='Session Management Testing'))
        Checklist.objects.create(name='Testing for Session Hijacking',
                                 parent=Checklist.objects.get(name='Session Management Testing'))
        Checklist.objects.create(name='Input Validation Testing',
                                 parent=Checklist.objects.get(name='Testing for Session Hijacking'))
        Checklist.objects.create(name='Testing for Reflected Cross Site Scripting',
                                 parent=Checklist.objects.get(name='Input Validation Testing'))
        Checklist.objects.create(name='Testing for Stored Cross Site Scripting',
                                 parent=Checklist.objects.get(name='Input Validation Testing'))
        Checklist.objects.create(name='Testing for HTTP Verb Tampering',
                                 parent=Checklist.objects.get(name='Input Validation Testing'))
        Checklist.objects.create(name='Testing for HTTP Parameter Pollution',
                                 parent=Checklist.objects.get(name='Input Validation Testing'))
        Checklist.objects.create(name='Testing for SQL Injection',
                                 parent=Checklist.objects.get(name='Input Validation Testing'))
        Checklist.objects.create(name='Testing for Oracle', parent=Checklist.objects.get(name='Testing for SQL Injection'))
        Checklist.objects.create(name='Testing for MySQL', parent=Checklist.objects.get(name='Testing for SQL Injection'))
        Checklist.objects.create(name='Testing for SQL Server',
                                 parent=Checklist.objects.get(name='Testing for SQL Injection'))
        Checklist.objects.create(name='Testing PostgreSQL', parent=Checklist.objects.get(name='Testing for SQL Injection'))
        Checklist.objects.create(name='Testing for MS Access',
                                 parent=Checklist.objects.get(name='Testing for SQL Injection'))
        Checklist.objects.create(name='Testing for NoSQL Injection',
                                 parent=Checklist.objects.get(name='Testing for SQL Injection'))
        Checklist.objects.create(name='Testing for ORM Injection',
                                 parent=Checklist.objects.get(name='Testing for SQL Injection'))
        Checklist.objects.create(name='Testing for Client-side',
                                 parent=Checklist.objects.get(name='Testing for SQL Injection'))
        Checklist.objects.create(name='Testing for LDAP Injection',
                                 parent=Checklist.objects.get(name='Testing for Client-side'))
        Checklist.objects.create(name='Testing for XML Injection',
                                 parent=Checklist.objects.get(name='Testing for Client-side'))
        Checklist.objects.create(name='Testing for SSI Injection',
                                 parent=Checklist.objects.get(name='Testing for Client-side'))
        Checklist.objects.create(name='Testing for XPath Injection',
                                 parent=Checklist.objects.get(name='Testing for Client-side'))
        Checklist.objects.create(name='Testing for IMAP SMTP Injection',
                                 parent=Checklist.objects.get(name='Testing for Client-side'))
        Checklist.objects.create(name='Testing for Code Injection',
                                 parent=Checklist.objects.get(name='Testing for Client-side'))
        Checklist.objects.create(name='Testing for Local File Inclusion',
                                 parent=Checklist.objects.get(name='Testing for Code Injection'))
        Checklist.objects.create(name='Testing for Remote File Inclusion',
                                 parent=Checklist.objects.get(name='Testing for Code Injection'))
        Checklist.objects.create(name='Testing for Command Injection',
                                 parent=Checklist.objects.get(name='Testing for Remote File Inclusion'))
        Checklist.objects.create(name='Testing for Format String Injection',
                                 parent=Checklist.objects.get(name='Testing for Remote File Inclusion'))
        Checklist.objects.create(name='Testing for Incubated Vulnerability',
                                 parent=Checklist.objects.get(name='Testing for Remote File Inclusion'))
        Checklist.objects.create(name='Testing for HTTP Splitting Smuggling',
                                 parent=Checklist.objects.get(name='Testing for Remote File Inclusion'))
        Checklist.objects.create(name='Testing for HTTP Incoming Requests',
                                 parent=Checklist.objects.get(name='Testing for Remote File Inclusion'))
        Checklist.objects.create(name='Testing for Host Header Injection',
                                 parent=Checklist.objects.get(name='Testing for Remote File Inclusion'))
        Checklist.objects.create(name='Testing for Server-side Template Injection',
                                 parent=Checklist.objects.get(name='Testing for Remote File Inclusion'))
        Checklist.objects.create(name='Testing for Server-Side Request Forgery',
                                 parent=Checklist.objects.get(name='Testing for Remote File Inclusion'))
        Checklist.objects.create(name='Testing for Error Handling',
                                 parent=Checklist.objects.get(name='Testing for Server-Side Request Forgery'))
        Checklist.objects.create(name='Testing for Improper Error Handling',
                                 parent=Checklist.objects.get(name='Testing for Error Handling'))
        Checklist.objects.create(name='Testing for Stack Traces',
                                 parent=Checklist.objects.get(name='Testing for Error Handling'))
        Checklist.objects.create(name='Testing for Weak Cryptography',
                                 parent=Checklist.objects.get(name='Testing for Stack Traces'))
        Checklist.objects.create(name='Testing for Weak Transport Layer Security',
                                 parent=Checklist.objects.get(name='Testing for Weak Cryptography'))
        Checklist.objects.create(name='Testing for Padding Oracle',
                                 parent=Checklist.objects.get(name='Testing for Weak Cryptography'))
        Checklist.objects.create(name='Testing for Sensitive Information Sent via Unencrypted Channels',
                                 parent=Checklist.objects.get(name='Testing for Weak Cryptography'))
        Checklist.objects.create(name='Testing for Weak Encryption',
                                 parent=Checklist.objects.get(name='Testing for Weak Cryptography'))
        Checklist.objects.create(name='Business Logic Testing',
                                 parent=Checklist.objects.get(name='Testing for Weak Encryption'))
        Checklist.objects.create(name='Introduction to Business Logic',
                                 parent=Checklist.objects.get(name='Business Logic Testing'))
        Checklist.objects.create(name='Test Business Logic Data Validation',
                                 parent=Checklist.objects.get(name='Business Logic Testing'))
        Checklist.objects.create(name='Test Ability to Forge Requests',
                                 parent=Checklist.objects.get(name='Business Logic Testing'))
        Checklist.objects.create(name='Test Integrity Checks', parent=Checklist.objects.get(name='Business Logic Testing'))
        Checklist.objects.create(name='Test for Process Timing',
                                 parent=Checklist.objects.get(name='Business Logic Testing'))
        Checklist.objects.create(name='Test Number of Times a Function Can Be Used Limits',
                                 parent=Checklist.objects.get(name='Business Logic Testing'))
        Checklist.objects.create(name='Testing for the Circumvention of Work Flows',
                                 parent=Checklist.objects.get(name='Business Logic Testing'))
        Checklist.objects.create(name='Test Defenses Against Application Misuse',
                                 parent=Checklist.objects.get(name='Business Logic Testing'))
        Checklist.objects.create(name='Test Upload of Unexpected File Types',
                                 parent=Checklist.objects.get(name='Business Logic Testing'))
        Checklist.objects.create(name='Test Upload of Malicious Files',
                                 parent=Checklist.objects.get(name='Business Logic Testing'))
        Checklist.objects.create(name='Client-side Testing',
                                 parent=Checklist.objects.get(name='Test Upload of Malicious Files'))
        Checklist.objects.create(name='Testing for DOM-Based Cross Site Scripting',
                                 parent=Checklist.objects.get(name='Client-side Testing'))
        Checklist.objects.create(name='Testing for JavaScript Execution',
                                 parent=Checklist.objects.get(name='Client-side Testing'))
        Checklist.objects.create(name='Testing for HTML Injection',
                                 parent=Checklist.objects.get(name='Client-side Testing'))
        Checklist.objects.create(name='Testing for Client-side URL Redirect',
                                 parent=Checklist.objects.get(name='Client-side Testing'))
        Checklist.objects.create(name='Testing for CSS Injection', parent=Checklist.objects.get(name='Client-side Testing'))
        Checklist.objects.create(name='Testing for Client-side Resource Manipulation',
                                 parent=Checklist.objects.get(name='Client-side Testing'))
        Checklist.objects.create(name='Testing Cross Origin Resource Sharing',
                                 parent=Checklist.objects.get(name='Client-side Testing'))
        Checklist.objects.create(name='Testing for Cross Site Flashing',
                                 parent=Checklist.objects.get(name='Client-side Testing'))
        Checklist.objects.create(name='Testing for Clickjacking', parent=Checklist.objects.get(name='Client-side Testing'))
        Checklist.objects.create(name='Testing WebSockets', parent=Checklist.objects.get(name='Client-side Testing'))
        Checklist.objects.create(name='Testing Web Messaging', parent=Checklist.objects.get(name='Client-side Testing'))
        Checklist.objects.create(name='Testing Browser Storage', parent=Checklist.objects.get(name='Client-side Testing'))
        Checklist.objects.create(name='Testing for Cross Site Script Inclusion',
                                 parent=Checklist.objects.get(name='Client-side Testing'))
        Checklist.objects.create(name='API Testing',
                                 parent=Checklist.objects.get(name='Testing for Cross Site Script Inclusion'))
        Checklist.objects.create(name='Testing GraphQL', parent=Checklist.objects.get(name='API Testing'))
        Checklist.objects.create(name='Reporting', parent=root)
...
def ataque(request):
    ...
    return render(request, "ataque.html", {'genres': Checklist.objects.all(),"teste":Checklist.objects.filter(level=0)})


@login_required(login_url='/login/')
def ataqueId(request,id):

    return render(request, "ataque.html", {'genres': Checklist.objects.filter(tree_id=int(id)),"teste":Checklist.objects.filter(level=0)})

@login_required(login_url='/login/')
def defesa(request):
    return render(request, 'defesa.html')
...
def scanOpcoes(request):
    ...
    verificar_continuacao = portas.split("-")


    try:
        porta_vai = " "+ verificar_continuacao[0] + "-" + verificar_continuacao[1]
    except:
...
def lerArquivoXml(dataAgora,usuario,arquivo):
    ...
    for vuln in vulnerabilidades_vetor:
        ...
        score = vuln.CVSS
        try:
            Vulnerabilidades.objects.get(ip=ipObjeto,porta=portaIp,tipo=vuln.tipo,path=vuln.path,parametro=vuln.parametro,CVSS=score,impacto=vuln.impacto,recomendacao=vuln.recomendacao,grau=vuln.grau,usuario=User.objects.get(username=usuario))
        except:
            Vulnerabilidades.objects.create(ip=ipObjeto,porta=portaIp,tipo=vuln.tipo,path=vuln.path,parametro=vuln.parametro,CVSS=score,impacto=vuln.impacto,recomendacao=vuln.recomendacao,grau=vuln.grau,usuario=User.objects.get(username=usuario))
...
def ligarMetaSploi(request):
    ...
    return render(request,'tela_visualização_exploracao.html',dados)
...
def cofre_submit(request):
    ...
    try:
        ...
        if (cofre.hash == hash_texto_chave):
            return render(request, 'cofre_ver.html',{'senha':key,'cofre':Arquivo.objects.filter(hash__usuario=request.user),
...
def cofre(request):
    ...
    return render(request,'cofre.html',dados)




def relatorio_completo(request,id):
    pentest_objeto = Pentest_Rede.objects.get(id=id)
    ...
    if pentest_objeto.usuario == request.user:
        ...
        class Relatorio_vai:
            def __init__(self, baixa, intermediaria,alta,critica,ip,portas,vulnerabilidades,cve,hash_imagem,hash_imagem2,sistemas):
                self.baixa = baixa
                self.intermediaria = intermediaria
                self.alta = alta
                ...
                self.hash_imagem2 = hash_imagem2
                self.sistemas = sistemas
    relatorios = []
    for ip in ips:
        rede_objeto = pentest_objeto.rede
        ip_vai = IP.objects.get(ip=ip.ip,rede=rede_objeto)
        ...
        relatorios.append(Relatorio_vai(baixa,intermediaria,alta,critica,ip_vai,portas_vai,vulnerabilidades,CVE_IP.objects.filter(ip=ip_vai),hash_veio,hash_veio2,sistema_vai))
    ...
    return render(request, 'postagens2.html', nome)
...
def submitFotoEvidencia(request,id):
    ...
    try:
        ...
        return HttpResponse("Coloca uma imagem ai amigão")
    EvidenciaCheckList.objects.create(descricao = descricao,
                         titulo = titulo,
                                      usuario=request.user,
                                      check=id,
...
def forum(request,id):
    forum = ChecklistRede.objects.get(id=id)

    evidencias = EvidenciaCheckList.objects.filter(check=int(id))
    print(evidencias)
    dados = {'forum': forum,'postagens':PostagensForum.objects.filter(check=int(id)),
             'evidencias':evidencias}
    return render(request,'forum.html',dados)


@login_required(login_url='/login/')
def comunidade(request,id):
    forum = Vulnerabilidades.objects.get(id=id)

    evidencias = PostagensForum.objects.filter(check=int(id))
    print(evidencias)
    dados = {'forum': forum,'postagens':PostagensForum.objects.filter(check=int(id)),
             'evidencias':evidencias,'redes':Pentest_Rede.objects.filter(usuario=request.user)}
    return render(request,'comunidade.html',dados)


@login_required(login_url='/login/')
def comunidade_vuln(request,id):
    comentario = request.POST.get('comentario')
    pai = request.POST.get('pai')
    if pai == None:
        PostagensForum.objects.create(name=comentario,usuario=request.user,check=id)
    else:
        PostagensForum.objects.create(name=comentario,usuario=request.user,check=id,parent=PostagensForum.objects.get(name=pai))


    return redirect("/vulnerabilidade/id/" + id)
...
class VulnerabilidadesViewSet(viewsets.ModelViewSet):
    authentication_classes = [SessionAuthentication, BasicAuthentication]
    permission_classes = [DjangoModelPermissionsOrAnonReadOnly, UserPermission]

    queryset = Vulnerabilidades.objects.all()
    serializer_class = VulnerabilidadesSerializer
    http_method_names = ['get', 'post', 'put', 'path','delete']


@login_required(login_url='/login/')
def mudarTratamentoVuln(request,id):

    vuln = Vulnerabilidades.objects.get(id=id)
    if int(vuln.tratada) < 3:
        vuln.tratada = int(vuln.tratada) + 1
        vuln.save()

        Vulnerabilidades_Tratamento.objects.create(Vulnerabilidades = vuln,usuario=request.user,tratada= int(vuln.tratada))
    return redirect("/vulnerabilidade/id/" + id)